...
class TextWinni:
    def __init__(self, locator, saver, sound_player):
        ...
        if saver.load_state()["sound_mode"] == "winni":
            self.is_active = True
        size = int(1 * locator.block_size[0])
        position = (locator.shift_horizontal + int(21 * locator.block_size[0])//2,
    ...
    def update(self):
        ...
        self.is_active = False
        if self.saver.load_state()["sound_mode"] == "winni":
            self.is_active = True
        ...
        if self.rect.collidepoint(mouse_cursor):
            ...
            if pressed[0]:
                self.saver.save_sound_mode("winni")
