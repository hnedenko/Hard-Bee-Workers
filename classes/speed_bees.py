...
class SpeedBees(pygame.sprite.Sprite):
    def __init__(self, locator, all_sprites):
        ...
        position = (locator.shift_horizontal + size[0]//2 + int(1.5 * locator.block_size[0]),
                    locator.shift_vertical + size[1]//2 + int(6.5 * locator.block_size[1]))
        ...
        image = pygame.image.load(path.join(images_dir, "bee_speed.png")).convert_alpha()
